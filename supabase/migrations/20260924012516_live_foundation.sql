...
create table public.live_participants (
  ...
  participation_state text not null
    check (participation_state in ('spectator','pending_request','pending_invitation','active','voluntary_exit','removed','rejected','finished')),
  window_slot integer,
  ...
  constraint live_participants_window_slot_check
    check (window_slot is null or window_slot between 0 and 11),
  constraint live_participants_state_timestamps_check
    check (
...
create table public.live_invitations (
  ...
  constraint live_invitations_response_timestamp_check
    check (
...
create table public.live_join_requests (
  ...
  constraint live_join_requests_decision_check
    check (
