    ...
    if (!messageError && otherUserId) {
      await supabase.from("notifications").insert({
        ...
        data: {
          ...
          otherUsername: user.user_metadata?.username ?? "", otherAvatar: "",
        },
      });
    }
      ...
      <View style={styles.header}>
        ...
        <View style={styles.callActions}>
          <TouchableOpacity style={styles.headerAction}
            onPress={() => void startCall("voice")} disabled={startingCall !== null}>
            {startingCall === "voice" ? <ActivityIndicator size="small" color="#fff" /> :
      ...
      <KeyboardAvoidingView style={{ flex: 1 }}
        ...
        {loading ? (
          ...
          <FlatList ref={flatListRef} data={messages} keyExtractor={(item) => item.id}
            renderItem={renderItem} contentContainerStyle={styles.messageList}
            showsVerticalScrollIndicator={false}
            onContentSizeChange={() => flatListRef.current?.scrollToEnd({ animated: false })} />
        )}

        <View style={[styles.inputRow, { paddingBottom: botPad + 8 }]}>
          <TextInput value={text} onChangeText={setText} placeholder="Mensaje..."
            placeholderTextColor="#555" style={styles.input} multiline maxLength={1000}
...
const styles = StyleSheet.create({
  ...
  inputRow: {
    ...
    borderTopWidth: 1, borderTopColor: "#111", gap: 10,
  },
  input: {
    flex: 1, backgroundColor: "#1C1C1E", borderRadius: 22, paddingHorizontal: 16,
    paddingVertical: 10, color: "#fff", fontSize: 15, maxHeight: 120,
  },
  sendBtn: {
    width: 42, height: 42, borderRadius: 21, backgroundColor: "#FE2C55",
